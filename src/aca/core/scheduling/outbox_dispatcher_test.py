...
class OutboxDispatcherTest(unittest.TestCase):
    ...
    def test_dispatches_status_and_comment_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("src.aca.core.scheduling.outbox_dispatcher.ensure_github_mcp_connected", return_value=None):
                with patch("src.aca.core.scheduling.outbox_dispatcher.ensure_github_mcp_disconnected", return_value=None):
                    with patch("src.aca.core.scheduling.outbox_dispatcher.update_project_item_status", return_value=None) as status_mock:
                        with patch("src.aca.core.scheduling.outbox_dispatcher.add_issue_comment", return_value=None) as comment_mock:
    ...
    def test_terminal_failure_marks_outbox_failed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("src.aca.core.scheduling.outbox_dispatcher.ensure_github_mcp_connected", return_value=None):
                with patch("src.aca.core.scheduling.outbox_dispatcher.ensure_github_mcp_disconnected", return_value=None):
                    with patch(
                        "src.aca.core.scheduling.outbox_dispatcher.update_project_item_status",
    ...
    def test_dispatches_pull_request_create_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("src.aca.core.scheduling.outbox_dispatcher.ensure_github_mcp_connected", return_value=None):
                with patch("src.aca.core.scheduling.outbox_dispatcher.ensure_github_mcp_disconnected", return_value=None):
                    with patch("src.aca.core.scheduling.outbox_dispatcher.create_pull_request", return_value="https://github.com/frumu-ai/example/pull/7") as pr_mock:
                        summary = dispatch_outbox_tick(cfg, coordination=store)
    ...
    def test_dispatch_tick_runs_for_postgres_backend(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("src.aca.core.scheduling.outbox_dispatcher.CoordinationStore.from_config", return_value=store):
                with patch("src.aca.core.scheduling.outbox_dispatcher.ensure_github_mcp_connected", return_value=None):
                    with patch("src.aca.core.scheduling.outbox_dispatcher.ensure_github_mcp_disconnected", return_value=None):
                        with patch("src.aca.core.scheduling.outbox_dispatcher.update_project_item_status", return_value=None):
