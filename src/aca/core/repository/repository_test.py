...
from src.aca.core.engine.process_utils import run_command
from src.aca.core.repository.repository import (
...
class RepositoryNamingTest(unittest.TestCase):
    ...
    def test_resolve_repository_clones_into_explicit_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run_command(["git", "-C", str(source), "-c", "user.name=ACA", "-c", "user.email=aca@local.invalid", "add", "README.md"])
            run_command(["git", "-C", str(source), "-c", "user.name=ACA", "-c", "user.email=aca@local.invalid", "commit", "-m", "init"])
    ...
    def test_resolve_repository_fast_forwards_existing_explicit_checkout(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run_command(["git", "-C", str(source), "-c", "user.name=ACA", "-c", "user.email=aca@local.invalid", "add", "README.md"])
            run_command(["git", "-C", str(source), "-c", "user.name=ACA", "-c", "user.email=aca@local.invalid", "commit", "-m", "one"])
            run_command(["git", "clone", str(source), str(target)])
            (source / "README.md").write_text("two\n", encoding="utf-8")
            run_command(["git", "-C", str(source), "-c", "user.name=ACA", "-c", "user.email=aca@local.invalid", "commit", "-am", "two"])
            (root / "agent.yaml").write_text(
                "\n".join(
                    [
                        "agent:",
