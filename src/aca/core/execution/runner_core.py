...
from src.aca.core.repository.board import card_to_task, claim_card, move_card, save_board, select_card
...
from src.aca.core.shutdown import ShutdownHandler
from src.aca.core.engine.coder_backend import (
    build_coder_summary,
    coder_backend_mode,
    coder_workflow_supported,
    execute_coder_run,
)
from src.aca.core.engine.engine import checkout_run_branch, commit_repository_changes, effective_tandem_provider, engine_env, engine_health, ensure_engine, git_diff_stat, push_repository_changes, resolve_repository, task_run_branch_name, write_provider_override_config
...
from src.aca.core.repository.repository import repository_binding_issues
from src.aca.core.scheduling.outbox_dispatcher import dispatch_outbox_tick
from src.aca.core.scheduling.coder_supervisor import apply_coder_result
from src.aca.core.verification.review_policy import evaluate_review_policy
from src.aca.core.verification.verification_policy import evaluate_verification_policy, review_blocker_message, test_blocker_message
from src.aca.core.engine.prompts import build_integration_prompt, build_manager_prompt, build_qa_prompt, build_review_prompt, build_test_prompt, derive_subtasks
...
from src.aca.core.execution.run_lifecycle import (
    block_run,
    build_provider_config_dict,
    build_swarm_config_dict,
    make_run_result,
)
from src.aca.core.phases.engine_check import (
    check_engine_at_startup,
    check_engine_health,
    resolve_repo_after_checkout,
)
from src.aca.core.phases.planning import (
    pre_screen_subtasks,
    run_manager_prompt,
)
from src.aca.core.phases.review_verify import run_review_and_test
from src.aca.core.phases.finalize import finalize_completed_run
from src.aca.core.phases.github_sync import (
    connect_for_intake,
    disconnect_for_coding,
    sync_claim_status,
    finalize_sync,
)
from src.aca.core.phases.task_intake import run_task_intake
from src.aca.core.phases.worker_dispatch import dispatch_workers
from src.aca.core.phases.repair import (
    RepairDecision,
    build_retry_feedback,
    check_no_diff,
    check_no_verifiable_proof,
)
from src.aca.core.phases.context import RunContext as _PhaseRunContext
from src.aca.runtime.run_output import build_blocked_summary, build_completed_summary, save_run_text, set_status, write_board_snapshot, write_blackboard_snapshot, write_diff_snapshot
from src.aca.runtime.artifact_store import configure_artifact_store_root
from src.aca.runtime.runstate import append_event, ensure_layout, initial_blackboard, initial_status, load_status, new_run_id, save_blackboard, write_status
from src.aca.runtime.task_sources import invalidate_cached_github_project_board_snapshot, normalize_task
from src.aca.utils.utils import slugify
from src.aca.core.execution.worker import run_worker_subtask, stream_tandem_prompt, sync_worker_artifacts
        ...
        if result is None:
            ...
            if ctx is not None:
                ...
                if (
                    ...
                    try:
                        from src.aca.runtime.runstate import append_event
...
def _run_once_internal_impl(
    ...
    ``src/aca/core/phases/``.  The RunContext object carries all shared
...
def _run_coder_backend(ctx: "_PhaseRunContext") -> dict[str, Any]:
    ...
    from src.aca.core.engine.coder_backend import coder_workflow_supported, execute_coder_run
    from src.aca.runtime.run_output import (
        build_coder_summary,
    ...
    from src.aca.core.repository.board import save_board
...
def _block_no_targets(ctx: "_PhaseRunContext") -> dict[str, Any]:
    ...
    from src.aca.runtime.run_output import build_blocked_summary, save_run_text, set_status, write_board_snapshot, write_blackboard_snapshot
    from src.aca.core.repository.board import save_board
...
def _block_manager_failed(ctx: "_PhaseRunContext") -> dict[str, Any]:
    ...
    from src.aca.runtime.run_output import build_blocked_summary, save_run_text
...
def _complete_pre_satisfied(ctx: "_PhaseRunContext") -> dict[str, Any]:
    ...
    from src.aca.core.engine.engine import git_diff_stat
    from src.aca.runtime.run_output import (
        build_completed_summary, save_run_text, set_status, write_board_snapshot, write_blackboard_snapshot
    )
    from src.aca.core.repository.board import save_board
    from src.aca.runtime.runstate import save_blackboard
...
def _block_from_decision(ctx: "_PhaseRunContext", decision: "RepairDecision") -> dict[str, Any]:
    ...
    from src.aca.runtime.run_output import build_blocked_summary, save_run_text, set_status, write_blackboard_snapshot
    from src.aca.runtime.runstate import save_blackboard
