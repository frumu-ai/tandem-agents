...
from src.aca.utils.utils import atomic_write_yaml, load_yaml, now_ms, short_id, slugify

WORKSPACE_DIR_NAME = ".ACA"
...
def _normalize_workspace(workspace: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "name": _as_text(data.get("name"), "ACA Workspace"),
...
def default_workspace(root: Path) -> dict[str, Any]:
    return {
        "workspace": {
            ...
            "name": Path(root).expanduser().name or "ACA Workspace",
            "created_at_ms": now_ms(),
            "updated_at_ms": now_ms(),
            "projects": [],
            "runs": [],
