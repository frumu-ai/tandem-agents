...
content="$(
  {
    ...
    emit HOSTED_RUNTIME_SECURITY_VERSION "${HOSTED_RUNTIME_SECURITY_VERSION:-1}"
    emit HOSTED_TANDEM_ENGINE_SOURCE_REVISION "${HOSTED_TANDEM_ENGINE_SOURCE_REVISION:-}"
    emit HOSTED_POLICY_ROOT "${HOSTED_POLICY_ROOT:-${install_root}/hosted-policy}"
    if [[ "${HOSTED_RUNTIME_SECURITY_VERSION:-1}" == "3" ]]; then
      emit HOSTED_ENGINE_BINARY_SHA256 "${HOSTED_ENGINE_BINARY_SHA256:-}"
      emit HOSTED_ENGINE_ATTESTATION_SHA256 "${HOSTED_ENGINE_ATTESTATION_SHA256:-}"
      emit HOSTED_MEMORY_KMS_COMMAND_ROOT "${HOSTED_MEMORY_KMS_COMMAND_ROOT:-${install_root}/memory-kms-commands}"
      emit HOSTED_MEMORY_ENCRYPTION_REQUIRED "${HOSTED_MEMORY_ENCRYPTION_REQUIRED:-}"
      emit HOSTED_MEMORY_KMS_PROVIDER "${HOSTED_MEMORY_KMS_PROVIDER:-}"
      emit HOSTED_MEMORY_KMS_RUNTIME_PRINCIPAL_ID "${HOSTED_MEMORY_KMS_RUNTIME_PRINCIPAL_ID:-}"
      emit HOSTED_MEMORY_KMS_ENCRYPT_COMMAND "${HOSTED_MEMORY_KMS_ENCRYPT_COMMAND:-}"
      emit HOSTED_MEMORY_KMS_DECRYPT_COMMAND "${HOSTED_MEMORY_KMS_DECRYPT_COMMAND:-}"
      emit HOSTED_MEMORY_KEK_ID "${HOSTED_MEMORY_KEK_ID:-}"
      emit HOSTED_MEMORY_KEK_VERSION "${HOSTED_MEMORY_KEK_VERSION:-}"
      emit HOSTED_MEMORY_KEK_ROTATION_EPOCH "${HOSTED_MEMORY_KEK_ROTATION_EPOCH:-}"
    fi
