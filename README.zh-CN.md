...
- 启动服务后运行 `docker compose exec aca python3 -m src.tandem_agents.cli.cli run`。
...
- 编排代码：`src/tandem_agents/core/`
- CLI/API 入口：`src/tandem_agents/cli/` 和 `src/tandem_agents/api/`
- 运行时状态写入：`src/tandem_agents/runtime/`
- setup 和启动脚本：`scripts/`
